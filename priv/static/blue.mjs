  ...
  #createChild(parent, index5, vnode) {
    switch (vnode.kind) {
      ...
      case fragment_kind: {
        ...
        const head2 = createChildText(parent, index5, vnode);
        appendChild(node, head2);
...
var virtualise = (root3) => {
  ...
  if (vdom === null || vdom.children instanceof Empty) {
    ...
    const head2 = emptyTextNode(root3);
    root3.insertBefore(head2, root3.firstChild);
...
var uri_from_url = (url) => {
  const optional2 = (value) => value ? new Some(value) : new None();
  return new Uri(
    /* scheme   */
...
// build/dev/javascript/blue/blue.mjs
var FILEPATH = "src/blue.gleam";
var ApiReturnedFeed = class extends CustomType {
  constructor($0) {
    super();
    this[0] = $0;
  }
...
var FeedResponse = class extends CustomType {
  constructor(feed) {
    super();
    this.feed = feed;
  }
};
var FeedPost = class extends CustomType {
  constructor(post) {
    super();
    this.post = post;
  }
...
var Post2 = class extends CustomType {
  constructor(uri, author, record, embed) {
    super();
    this.uri = uri;
    this.author = author;
    this.record = record;
...
var Author = class extends CustomType {
  constructor(handle2, display_name, avatar) {
    super();
    this.handle = handle2;
...
var Record = class extends CustomType {
  constructor(created_at, text4) {
    super();
    this.created_at = created_at;
    this.text = text4;
  }
};
var ImagesEmbed = class extends CustomType {
  constructor(images) {
    super();
    this.images = images;
  }
      ...
      return field(
        ...
        (display_name) => {
          return field(
            ...
            string2,
            (avatar) => {
              return success(new Author(handle2, display_name, avatar));
            }
          );
        }
      );
  ...
  return field(
    ...
    (created_at) => {
      return field(
        ...
        string2,
        (text4) => {
          return success(new Record(created_at, text4));
        }
      );
    }
  );
}
function image_decoder() {
  return field(
    "thumb",
    string2,
    (thumb) => {
      return field(
        ...
        (fullsize) => {
          return field(
            ...
            string2,
            (alt2) => {
              return success(new Image(thumb, fullsize, alt2));
            }
          );
        }
      );
  ...
  return field(
    ...
    (embed_type) => {
      ...
      if (embed_type === "app.bsky.embed.images#view") {
        ...
        let t = embed_type;
        return success(new UnsupportedEmbed(t));
      }
    }
  );
}
...
function feed_post_decoder() {
  return field(
    "post",
    post_decoder(),
    (post) => {
      return success(new FeedPost(post));
    }
  );
}
function feed_response_decoder() {
  return field(
    "feed",
    list2(feed_post_decoder()),
    (feed) => {
      return success(new FeedResponse(feed));
    }
  );
}
function get_feed(name) {
  ...
  let handler = expect_json(
    ...
    (var0) => {
      return new ApiReturnedFeed(var0);
    }
  );
                  ...
                  ul(
                    ...
                    toList([
                      li(
                        toList([]),
                        toList([
                          strong(toList([]), toList([text3("Blue")]))
                        ])
                      )
                    ])
                  ),
                      ...
                      toList([
                        ...
                        footer(
                          ...
                          toList([
                            small(
                              toList([]),
                              toList([text3(post.post.record.created_at)])
                            )
                          ])
                        )
                      ])
...
function main2() {
  ...
  if (!($ instanceof Ok)) {
    throw makeError(
      ...
      { value: $, start: 338, end: 387, pattern_start: 349, pattern_end: 354 }
    );
  }
  return void 0;
}
...
function echo$inspectDict(map6) {
  ...
  key_value_pairs.forEach(([key, value]) => {
    ...
    body2 = body2 + "#(" + echo$inspect(key) + ", " + echo$inspect(value) + ")";
    first = false;
  });
  return body2 + "])";
}
